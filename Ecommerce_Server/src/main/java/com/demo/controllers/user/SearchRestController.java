...
public class SearchRestController {
	...
	@RequestMapping(value = {"category/{id}" }, method = RequestMethod.GET, produces = MimeTypeUtils.APPLICATION_JSON_VALUE)
	public ResponseEntity<List<ProductInfo>> getProductbyCategory(@PathVariable("id") int id) {
		try {
			return new ResponseEntity<List<ProductInfo>>(productService.getProductbyCategory(id), HttpStatus.OK);
		} catch (Exception e) {
			return new ResponseEntity<List<ProductInfo>>(HttpStatus.BAD_REQUEST);
		}
	}
