...
public interface CartsRepositories extends CrudRepository<Carts, Integer> {
	...
	@Query("select new com.demo.models.CartsInfor(id, products.id , users.id , quantity, price, created) from Carts where id = :id ")
	public CartsInfor findbyId(@Param("id") int id);
