										...
										<tbody>
											<c:forEach var="item" items="${items }" varStatus="varStatus">
												...
												<td>${item.phone }</td>
													...
													<td>
													<a
													href="${pageContext.request.contextPath }/uploads/images/${item.logo }"
													data-toggle="lightbox" data-title="${item.name }'s logo">
                        								<img
														src="${pageContext.request.contextPath }/uploads/images/${item.logo }"
														class="img-fluid mb-2" alt="${item.name }'s logo"
														width="200px" />
                      								</a>
													</td>
													<td><fmt:formatDate var="created"
														...
														${created }</td>
													<td><fmt:formatDate var="banTerm"
														value="${item.banTerm }" pattern="dd/MM/yyyy" />
														${banTerm }</td>
													<td>${item.status ? "Enable" : "Disable" }</td>
													<td>
													<a
													href="${pageContext.request.contextPath }/manager/store/edit/${item.id }">
													...
													<a
													href="${pageContext.request.contextPath }/manager/store/${item.status ? 'ban' : 'toggleStatus' }/${item.id }">
															<button type="button"
		...
		<div class="modal-dialog">
			<div class="modal-content bg-danger">
				...
				<div class="modal-footer justify-content-between">
					...
					<a id="deleteLink"
							data-link="${pageContext.request.contextPath }/manager/store/delete/"
							href="#">
